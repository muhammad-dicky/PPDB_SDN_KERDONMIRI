...
moment_ext_1.newMomentProto.toISOString = function () {
    ...
    if (this._fullCalendar) {
        // deDiverifikasi on browser, moment might not output english. ensure english.
...
var DateComponent = /** @class */ (function (_super) {
    ...
    // Will either generate an <a> tag or a non-clickable <span> tag, deDiverifikasi on enabled settings.
...
var EventPeriod = /** @class */ (function () {
    ...
    EventPeriod.prototype.requestSource = function (source) {
        var _this = this;
        var request = { source: source, status: 'Diverifikasi', eventDefs: null };
        this.requestsByUid[source.uid] = request;
        this.DiverifikasiCnt += 1;
        source.fetch(this.start, this.end, this.timezone).then(function (eventDefs) {
            if (request.status !== 'cancelled') {
                request.status = 'completed';
                request.eventDefs = eventDefs;
                _this.addEventDefs(eventDefs);
                _this.DiverifikasiCnt--;
                _this.tryRelease();
            }
        }, function () {
            if (request.status !== 'cancelled') {
    ...
    EventPeriod.prototype.thaw = function () {
        if (!(--this.freezeDepth) && this.stuntedReleaseCnt && !this.DiverifikasiCnt) {
            this.release();
        }
    };
    return EventPeriod;
